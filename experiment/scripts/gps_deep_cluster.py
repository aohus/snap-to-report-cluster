...
class StructureScorer:
    ...
    def __init__(self, device: torch.device | None = None) -> None:
        ...
        texts = [
            "a scene with buildings, walls, stairs, benches, fences, paved roads, trees",
...
class LoFTRMatcher:
    def __init__(self, confidence_threshold: float = 0.8, pretrained: str = "outdoor", device: Optional[str] = DEVICE):
        self.confidence_threshold = confidence_threshold
        self.loftr = KF.LoFTR(pretrained=pretrained).to(device)
        self.device = device

    def _load_gray_tensor(self, path: Path) -> Optional[torch.Tensor]:
        ...
        # kornia expects tensor in range [0, 1]
        return kornia.image_to_tensor(img, keepdim=False).float() / 255.0

    @torch.no_grad()
    def geo_score(self, path1: Path, path2: Path) -> float:
        img1 = self._load_gray_tensor(path1)
        img2 = self._load_gray_tensor(path2)
        if img1 is None or img2 is None:
            return 0.0

        input_dict = {"image0": img1.to(self.device), "image1": img2.to(self.device)}
        correspondences = self.loftr(input_dict)

        confidences = correspondences['confidence'].cpu().numpy()
        num_confident_matches = np.sum(confidences > self.confidence_threshold)
        
        # Normalize the score. This is a heuristic.
        # The number of matches can vary a lot. 
        # A simple normalization could be to divide by a constant, 
        # or to use a function that saturates.
        # For now, let's use a simple scaling and clipping.
...
def create_people_detector(device=None):
...
class GeoParams:
    ...
    """
    matcher_type: str = "sift"
    max_features: Optional[int] = 1500
    ratio_thresh: Optional[float] = 0.75
    ransac_reproj_thresh: Optional[float] = 3.0
    min_good_matches: Optional[int] = 15
...
def create_deep_cluster(embed: EmbedParams, geo: GeoParams, scorer: dict, enable_debug: bool) -> DeepCluster:
    ...
    elif geo.matcher_type == "loftr":
        geo_matcher = LoFTRMatcher(
            confidence_threshold=geo.confidence_threshold,
            pretrained=geo.pretrained
        )
    else:
        raise ValueError(f"Unknown matcher_type: {geo.matcher_type}")
...
def build_experiment_grid() -> List[ExperimentConfig]:
    ...
    def make_combined() -> BaseDescriptorExtractor:
        ...
        ap = APGeMDescriptorExtractor(model_name="tf_efficientnet_b3_ns", image_size=320, people_detector=people_detector)
        cl = CLIPDescriptorExtractor(model_name="ViT-B-32", pretrained="openai", people_detector=people_detector)
        return CombinedAPGeMCLIPExtractor(apgem=ap, clip=cl, w_apgem=0.8, w_clip=0.2)

    embed_param_list = [
        EmbedParams(name="APGeM+CLIP", extractor_factory=make_combined, similarity_threshold=0.70, knn_k=8),
        EmbedParams(name="APGeM+CLIP", extractor_factory=make_combined, similarity_threshold=0.75, knn_k=10),
    ]
    ...
    for embed in embed_param_list:
        for geo in geo_param_list:
            exp_id = f"exp_{idx:03d}_{embed.name}_sim{embed.similarity_threshold}_k{embed.knn_k}_geo{geo.geo_threshold}_{geo.matcher_type}"
            configs.append(ExperimentConfig(id=exp_id, gps=gps_param_list[0], embed=embed, geo=geo))
            idx += 1
    return configs
